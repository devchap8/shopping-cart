...
export default function Store() {
    ...
    return (
        ...
        <div className={"h-14 flex items-center justify-around bg-ui-snow border-b-2 border-b-ui-muted sticky top-20 self-start"} data-testid="store-nav-bar">
            <select value={cont.currSort} onChange={(e) => cont.sortByPrice(e.target.value)} data-testid="select-sort" className={"h-10 text-xs sm:text-sm md:text-base border-2 border-ui-muted focus:border-ui-gray focus:outline-0 rounded-t-lg p-1 text-text-dim"} name="sortDropdown">
            ...
            <select value={cont.currCat} onChange={(e) => cont.filterByCat(e.target.value)} data-testid="select-filter" className={"h-10 text-xs sm:text-sm md:text-base border-2 border-ui-muted focus:border-ui-gray focus:outline-0 rounded-t-lg p-1 text-text-dim"} name="tagDropdown">
