            ...
            {cont.itemList.map(item => 
                <div key={item.name} className={"h-auto w-56 bg-ui-snow p-2 border-2 border-b-0 border-ui-muted rounded-2xl hover:shadow-2xl shadow-brand-lighter transition-shadow duration-500 ease-in-out"}>
                    ...
                    <form onSubmit={(e) => cont.addToCart(e, item.name)} className={"bg-brand-light -m-2 mt-2 p-1 pr-2 pl-2 h-10 rounded-b-2xl flex items-center"}>
                        <button onClick={(e) => cont.pressPlusMinus(e, true)} className={"border-2 border-r-0 w-6 font-bold hover:bg-brand-main rounded-l-lg"}>+</button>
                        <input className={"border-ui-dark border-t-2 border-b-2 text-center focus:outline-0 focus:bg-brand-lighter"} value={item.qty} type="number" min="0" max="99" name="qty" placeholder="qty" data-item-name={item.name}></input>
                        <button onClick={(e) => cont.pressPlusMinus(e, false)} className={"border-2 border-l-0 w-6 mr-auto font-bold hover:bg-brand-main rounded-r-lg"}>-</button>
